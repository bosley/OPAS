...
int main() {
    ...
    SERV::ObjTransport application("application_1", "127.0.0.1", 4096);

    // Create some objects that can receive information from the object server
    APP::Destination destination1("app_1_destination_1");
    APP::Destination destination2("app_1_destination_2");
    ...
    while(!timer.isExpired()) {
        ...
        if (sendTimer.isExpired()) {
            ...
            if (count % 2 == 0) {
                std::cout << "Application1 --> Application2.destination1" << std::endl;

                // If even, send to app 2 destination 1
                application.send("app_2_destination_1", obj);

            } else  {
                std::cout << "Application1 --> Application2.destination2" << std::endl;

                // If odd, send to app 2 destination 2
                application.send("app_2_destination_2", obj);
            }
