...
int main() {
    ...
    UTIL::Logger logger("RNNExample");
    logger.setLevel(LL_DEBUG);

    NNET::Builder networkBuilder(logger);

    NNET::Network network = networkBuilder.buildRecurrent(2, 4, 1, NNET::TransferFunction::TANH);
   ...
   // networkTrainer.trainByItr(network, getJsonFile("testTrain.json"), TRAINING_ITR, TRAINING_EPOCHS);
    ...
    LOG_DEBUG(logger) << incorrect << " of " << TEST_CASE_NUM 
                        << " xor inputs failed for examples rnn network after "
                        << TRAINING_ITR 
                        << " training iterations @ " 
                        << TRAINING_EPOCHS 
                        << " epochs per itr\n";
